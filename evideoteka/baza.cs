...
namespace evideoteka
{
    ...
    /*public List<korisnik> SelectMemberMemebership()
    {
        string query = "SELECT idDistributer, naziv FROM distributer";
        List<korisnik> list = new List<korisnik>();
        if (this.OpenConnection() == true)
        {
            ...
            while (dataReader.Read())
            {
                ...
                string pret = dataReader[0] + "";
                int idKor = Int32.Parse(pret);
                string smmUserName = dataReader[7] + "";
                korisnik1.idKorisnik = idKor;
                korisnik1.korisnickoIme = smmUserName;
                list.Add(korisnik1);
    ...
    public List<clanarina> SelectMembership()
    {
        ...
        if (this.OpenConnection() == true)
        {
            ...
            while (dataReader.Read())
            {
                ...
                model1.datumAktivacije = DateTime.Parse(prt3);
                string prt4 = dataReader[4] + "";
                model1.datumDeaktivacije = DateTime.Parse(prt4);
                list.Add(model1);
    ...
    public List<posudba> SelectLoan()
    {
        ...
        if (this.OpenConnection() == true)
        {
            ...
            while (dataReader.Read())
            {
                ...
                model1.datumPosudbe = DateTime.Parse(prt3);
                list.Add(model1);
